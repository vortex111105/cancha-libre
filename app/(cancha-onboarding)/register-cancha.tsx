...
export default function RegisterCanchaScreen() {
  ...
  async function handleRegister() {
    ...
    try {
      ...
      const { error: userError } = await supabase.from('cl_users').insert({
        id: userId,
        email: email.trim().toLowerCase(),
        role: 'cancha_owner',
      });
      ...
      router.replace('/(cancha-onboarding)/setup-field');
    } catch (err: any) {
      ...
      >
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.hero}>
            ...
            <Text style={styles.appName}>Portal de Canchas</Text>
            <Text style={styles.tagline}>Gestioná tu cancha y recibí reservas</Text>
          </View>

          <View style={styles.form}>
            <Text style={styles.title}>Crear cuenta</Text>
            <Text style={styles.subtitle}>Paso 1 de 2 — Datos del dueño</Text>
...
const styles = StyleSheet.create({
  ...
  subtitle: { fontSize: 13, color: Colors.primary, fontWeight: '600', marginTop: -8 },
  inputGroup: { gap: 6 },
